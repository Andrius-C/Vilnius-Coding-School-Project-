...
summary(train) # Find that humidity has some zero values, so will inspect it further
dim(train)
str(train)
...
#Both upper and lower known values have the same value, so will simply fill it with that value  
train$humidity[1092:1113] = train$humidity[1114] 
...
#and precition was in the top 10%(RMSLE_forest=0.40947) vs top %80(RMSLE_regression=0.76714) for regression 
