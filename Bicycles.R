...
#but the residual standard error is quite large at 94.85
...
fit_cas = lm(casual~ temp+humidity+timebin_cas
    +year+workingday+weather+season+holiday, data=train)
summary(fit_cas)
#Model fit is decent at R squared adjusted = 0.56 and 
#residual standard error is relatively lower than registered rents at 33
...
fancyRpartPlot(hours_reg)

hours_cas=rpart(casual~hour,train)
fancyRpartPlot(hours_cas)
...
#Received Root Mean Squared Logarithmic Error from Kaggle 
RMSLE_forest=0.40947 #which is top 10% of the competition 
